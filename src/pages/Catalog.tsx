...
const Catalog = () => {
  ...
  const cartKey = user ? `cartItems_${user.id}` : 'cartItems_guest';
  ...
  const getCategoryIconSVG = (petType: string) => {
    ...
    switch (petType) {
      ...
      case 'rodent':
        return `<svg xmlns="http://www.w3.org/2000/svg" class="${iconClass}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 9.5c0-3-4-3-4-3m-7 3.5.737-.271c.681-.25 1.263-.825 1.263-1.729 0-2-3-2-3 0 0 1-.5 2-2 3m-2 9-.871-.483c-.658-.365-1.129-1.112-1.129-1.897 0-2.02 3-2.528 3.5.395.667-2.138 0-4.5 0-6.5m8.5-1.5c0-1-1.5-2-1.5-2l1.5 1-1.5-3m-1 4 1.414-1.414"/><path d="M7 14.5a2.5 2.5 0 1 0 5 0 2.5 2.5 0 0 0-5 0Zm7-4a1.5 1.5 0 1 0 3 0 1.5 1.5 0 0 0-3 0Z"/></svg>`;
      default:
        return `<svg xmlns="http://www.w3.org/2000/svg" class="${iconClass}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="5" rx="2"/><path d="M16 2v4"/><path d="M8 2v4"/><path d="M2 10h20"/></svg>`;
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Navbar 
        cartItemCount={user ? cartItems.reduce((total, item) => total + item.quantity, 0) : 0} 
        currentPage="catalog"
      />
